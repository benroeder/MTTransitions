...
public class MTTransition: NSObject, MTIUnaryFilter {
    ...
    private func samplerImage(name: String) -> MTIImage? {
        // Under SwiftPM, `Bundle(for: MTTransition.self)` returns the
        // framework binary's enclosing bundle, which carries no
        // resources. The `.process("Assets.bundle")` declaration in
        // Package.swift (commit 3155d82) places the bundle into
        // `Bundle.module` (auto-generated by SwiftPM) — that's where
        // the lookup must succeed. Without this branch, transitions
        // that declare auxiliary textures via `samplers` (currently
        // `MTDisplacementTransition` and `MTLumaTransition`) fail to
        // bind their texture(2), and MetalPetal's
        // `MTIRenderPipelineKernel.m:342` NSAsserts on "Failed to set
        // fragment textures" in DEBUG builds — the NSException is
        // uncatchable from Swift and terminates the process via
        // SIGABRT. Same shape as `MTTransition.swiftPMLibraryURL`
        // shim that handles shader-library resolution for SPM.
        #if SWIFT_PACKAGE
